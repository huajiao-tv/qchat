...
@interface IMGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation IMGCDAsyncSocketPreBuffer
...
/**
 * The IMGCDAsyncReadPacket encompasses the instructions for any given read.
 * The content of a read packet allows the code to determine if we're:
 *  - reading to a certain length
 *  - reading to a certain separator
 *  - or simply reading the first chunk of available data
**/
@interface IMGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(IMGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation IMGCDAsyncReadPacket
...
/**
 * The IMGCDAsyncWritePacket encompasses the instructions for any given write.
**/
@interface IMGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation IMGCDAsyncWritePacket
...
/**
 * The IMGCDAsyncSpecialPacket encompasses special instructions for interruptions in the read/write queues.
 * This class my be altered to support more than just TLS in the future.
**/
@interface IMGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation IMGCDAsyncSpecialPacket
...
@implementation GCDAsyncSocket
{
	...
	IMGCDAsyncReadPacket *currentRead;
	IMGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	IMGCDAsyncSocketPreBuffer *preBuffer;
	...
	IMGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		preBuffer = [[IMGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
                        ...
                        tag:(long)tag
{
	...
	IMGCDAsyncReadPacket *packet = [[IMGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[IMGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[IMGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued IMGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued IMGCDAsyncReadPacket");
...
- (void)completeCurrentRead
{
	...
	{
		IMGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		IMGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	IMGCDAsyncWritePacket *packet = [[IMGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[IMGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[IMGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued IMGCDAsyncSpecialPacket");
			...
			{
				LogVerbose(@"Dequeued IMGCDAsyncWritePacket");
...
- (void)doWriteData
{
	...
	// Note: This method is not called if currentWrite is a IMGCDAsyncSpecialPacket (startTLS packet)
...
- (void)doWriteTimeout
{
	...
	{
		IMGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	IMGCDAsyncSpecialPacket *packet = [[IMGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			IMGCDAsyncSpecialPacket *tlsPacket = (IMGCDAsyncSpecialPacket *)currentRead;
...
- (void)ssl_startTLS
{
	...
	sslPreBuffer = [[IMGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[IMGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[IMGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
