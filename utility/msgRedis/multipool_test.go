...
func TestMultiPush(t *testing.T) {
	addresses := []string{"127.0.0.1:9931:1234567890", "127.0.0.1:9991:1234567890"}
	addr := "127.0.0.1:9991:1234567890"
	// addr := "127.0.0.1:9731"
	mp := NewMultiPool(addresses, 50, 20, 9)
	...
	// fmt.Println(mp.AddPool("127.0.0.1:9901", 10, 60))
...
func TestMultiPop(t *testing.T) {
	addresses := []string{"127.0.0.1:9731", "127.0.0.1:9991:1234567890"}
	// addr := "127.0.0.1:9991@1234567890"
	addr := "127.0.0.1:9731"
	mp := NewMultiPool(addresses, 50, 20, 9)
	fmt.Println(mp.AddPool("127.0.0.1:9901", 10, 20, 60))
	fmt.Println(mp.servers)
	go func() {
		for i := 0; i < 29; i++ {
			fmt.Println(mp.Info())
			// time.Sleep(time.Second)
			mp.AddPool("127.0.0.1:9901", 10, 10, 60)
			// mp.DelPool("127.0.0.1:9901")
			mp.ReplacePool("127.0.0.1:9901", "127.0.0.1:9801", 20, 10, 8)
		}
	}()
...
func TestMultiPool(t *testing.T) {
	...
	fmt.Println(mp.AddPool("127.0.0.1:9901", 10, 10, 60))
	go func() {
		for i := 0; i < 29; i++ {
			mp.Info()
			time.Sleep(time.Second)
