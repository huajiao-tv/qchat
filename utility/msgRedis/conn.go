...
func (c *Conn) GetPoolInfo() string {
	return fmt.Sprintf("%p %d %d %d %d", c, c.pool.ActiveNum, c.pool.IdleNum, len(c.pool.ClientPool), c.pool.CreateNum)
}
...
func (c *Conn) Call(command string, args ...interface{}) (interface{}, error) {
	...
	if c.readTimeout > 0 {
		if e = c.conn.SetReadDeadline(time.Now().Add(c.readTimeout)); e != nil {
			return nil, e
		}
	}
...
func (c *Conn) PipeExecWithPushBack() ([]interface{}, error) {
	var e error
