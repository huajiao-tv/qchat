...
func TestSigle(t *testing.T) {
	p := NewPool("127.0.0.1:9731", "", 10, 10, 10)
	c := p.Pop()
...
func TestPipeSmallBuffer(t *testing.T) {
	c, e := Dial("127.0.0.1:9731", "", ConnectTimeout, ReadTimeout, WriteTimeout, false, nil)
	if e != nil {
		println(e.Error())
		return
	}
...
func TestPoolAuth(t *testing.T) {
	p := NewPool("127.0.0.1:9991", "1234567890", 10, 10, 10)
...
func TestCommands(t *testing.T) {
	...
	fmt.Println(c.KEYS("*"))
	fmt.Println(c.MIGRATE("127.0.0.1", "9731", "key", "1", 5000, true, true))
